...
def render_music_box() -> None:
    ...
    #gunun_sarkisi = "https://open.spotify.com/embed/track/29Xdknl9fhRsV0oOYyQOKy" (Tek bir şarkı atamak istemediğimde burası işime yarayacak)
...
def render_teaser() -> None:
    teaser_html = """
    <div style="position: relative; width: 55%; height: 260px; border-radius: 18px; overflow: hidden; margin: 0 auto 26px auto; border: 1px solid rgba(201,166,224,0.22); box-shadow: 0 24px 48px -28px rgba(0,0,0,0.7); background: #1C1225;">
        <!-- Arka planda soluk, renksiz Türkiye silüeti (Doğrudan SVG kullanıldı) -->
        <div style="position: absolute; inset: 0; background-image: url('https://upload.wikimedia.org/wikipedia/commons/5/52/Turkey_location_map.svg'); background-size: contain; background-repeat: no-repeat; background-position: center; filter: grayscale(1) blur(1px); opacity: 0.35;"></div>
